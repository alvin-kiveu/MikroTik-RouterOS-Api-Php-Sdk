...
class PPPoE extends MikrotikSdk
{
    ...
    public function addPPPoESecret($ip, $login, $password, $name, $userpassword, $profile, $comment)
    {
        ...
        if ($connect) {
            ...
            $this->api->write('=password=' . $userpassword, false); // Include user password
            $this->api->write('=service=' . $service, false);
            $this->api->write('=profile=' . $profile, false);
            $this->api->write('=comment=' . $comment); // Add comment
            $pppoe = $this->api->read();
            $this->disconnect();
