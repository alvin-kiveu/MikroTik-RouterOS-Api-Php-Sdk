        ...
        if (is_array($response)) {
            ...
            foreach ($response as $x) {
                if (in_array($x, array('!fatal', '!re', '!trap'))) {
                    if ($x == '!re') {
                        $PARSED[] = array(); // Create a new element
                        $CURRENT = &$PARSED[count($PARSED) - 1]; // Reference the new last element
                    } else {
                        $PARSED[$x][] = array(); // Create a new element
                        $CURRENT = &$PARSED[$x][count($PARSED[$x]) - 1]; // Reference the new last element
                    }
                } elseif ($x != '!done') {
                    $MATCHES = array();
